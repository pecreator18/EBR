<template>
	<div class="test">
		<div class="batch_record_select" v-if = "br_selected === false">
			...
			<h3 v-else>Completed Batch Records</h3>
			<ol>
						<li v-for = "batch_record in batch_record_list">
							...
							<button class = "" id = "" v-on:click = "loadBatchRecord(batch_record.lot_number)">View Batch Record</button>
						</p>
						</li>
			</ol>
		</div>
...
import StepService from '../StepServices';
	...
	async created(){
		try{
			this.batch_record_list = await StepService.getCompletedBatchRecordList().then( (res) => { return res} );
